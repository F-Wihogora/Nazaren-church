          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <div className="flex flex-wrap justify-center gap-8 text-sm text-white/90">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium">Sunday: 8:00 AM – 12:00 PM</span>
              </div>
              <div className="flex items-center space-x-2">
          ...
          <div className="container mx-auto px-4 space-y-4">
            {announcements.map((a) => (
              ...
              >
                ...
                <p className="text-muted-foreground mb-2">{a.content}</p>
                {a.bibleVerse && (
                  <p className="text-sm italic text-primary font-medium">&quot;{a.bibleVerse}&quot;</p>
                )}
              </motion.div>
            ))}
          </div>
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {sermons.map((sermon, index) => (
                <motion.div key={sermon._id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }}>
                  <Card className="h-full hover:shadow-lg transition-shadow">
                    <CardHeader>
                      <CardTitle className="line-clamp-2">{sermon.title}</CardTitle>
                      <CardDescription>{sermon.preacher} • {new Date(sermon.date).toLocaleDateString()}</CardDescription>
                    </CardHeader>
                    <CardContent>
                      <Link href={`/sermons/${sermon._id}`}>
                        <Button className="w-full"><Play className="mr-2 h-4 w-4" /> Watch/Listen</Button>
                      </Link>
                    </CardContent>
                  </Card>
                </motion.div>
            ...
            <div className="text-center py-12 text-muted-foreground">No sermons available yet.</div>
          )}
        </div>
      </section>
          ...
          <div className="max-w-3xl mx-auto text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
              ...
              <p className="text-lg text-muted-foreground mb-8">
                Nazarene Church is a vibrant community of believers dedicated to serving God and spreading His love.
                We welcome everyone to join us in worship, fellowship, and spiritual growth.
              </p>
              <Link href="/about">
      ...
      <section className="py-20 bg-secondary/50">
        <div className="container mx-auto px-4 max-w-4xl">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            ...
            <p className="text-lg text-muted-foreground">We&apos;d love to hear from you. Visit us or reach out!</p>
          </motion.div>
              ...
              <CardContent>
                <div className="w-full h-64 rounded-lg overflow-hidden">
                  <iframe
                    ...
                    title="Remera Church of the Nazarene Map"
                  />
                </div>
              </CardContent>
            </Card>
