...
import {
  BookOpen, Calendar, Users, Building2, Bell, Heart, UserPlus, DollarSign,
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const [stats, setStats] = useState({
    sermons: 0, events: 0, members: 0, ministries: 0,
    announcements: 0, prayerRequests: 0, visitors: 0, givingTotal: 0,
  });
  ...
  const fetchStats = async () => {
    try {
      const [s, e, m, mn, a, p, v, g] = await Promise.all([
        fetch("/api/sermons"),
        fetch("/api/events"),
        fetch("/api/members"),
      ...
      const sermons = await s.json();
      const events = await e.json();
      const members = await m.json();
      const ministries = await mn.json();
      const announcements = await a.json();
      const prayer = await p.json();
      const visitors = await v.json();
      const giving = await g.json();
  ...
  const statCards = [
    ...
    { label: "Announcements", value: stats.announcements, icon: Bell, href: "/admin/announcements" },
    { label: "Prayer Requests", value: stats.prayerRequests, icon: Heart, href: "/admin/prayer-requests" },
    { label: "Visitors", value: stats.visitors, icon: UserPlus, href: "/admin/visitors" },
    { label: "Total Giving", value: `RWF ${stats.givingTotal.toLocaleString()}`, icon: DollarSign, href: "/admin/giving" },
  ];
